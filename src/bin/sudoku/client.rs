...
type Rx = mpsc::UnboundedReceiver<i32>;
...
async fn process_session(id: String, 
                         ...
                         pipelines: i32) -> Result<(), Box<dyn Error>> {
    ...
    let _ = session.send(pipelines).await?;
...
impl Session {
    ...
    fn report(&mut self) {
        ...
        if shared.remain == 0 {
            ...
            if let Err(e) = percentile.save(&shared.latencies, &format!("p{:04}", shared.count)) {
...
async fn main() -> io::Result<()> {
    ...
    if env::args().len() < 5 {
        error!("Usage: {} input server_addr connections pipelines", env::args().nth(0).unwrap());
        return Ok(());
    }
    
    let input = env::args().nth(1).unwrap();
    let server_addr = env::args().nth(2).unwrap();
    let connections: i32 = env::args().nth(3).unwrap().parse().unwrap();
    let pipelines: i32  = env::args().nth(4).unwrap().parse().unwrap();
    ...
    for id in 0 .. connections {
        ...
        tokio::spawn(async move {
            if let Err(e) = process_session((id+1).to_string(), addr, notify_shutdown_sub, shutdown_complete_tx_clone, rx, shared_clone, pipelines).await {
                tracing::info!("an error occurred when process_session; error = {:?}", e);
            }
        });
    }
